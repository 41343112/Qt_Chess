...
void Qt_Chess::setupUI() {
    ...
    boardContainerVLayout->setSpacing(2);

    // 對方的吃子紀錄（黑方吃掉的白子）- 水平排列在棋盤上方，靠近棋盤
    m_capturedWhitePanel = new QWidget(m_boardContainer);
    m_capturedWhitePanel->setMinimumWidth(100);
    m_capturedWhitePanel->setMinimumHeight(28);
    m_capturedWhitePanel->setMaximumHeight(32);
    ...
    // 我方的吃子紀錄（白方吃掉的黑子）- 水平排列在棋盤下方，靠近棋盤
    m_capturedBlackPanel = new QWidget(m_boardContainer);
    m_capturedBlackPanel->setMinimumWidth(100);
    m_capturedBlackPanel->setMinimumHeight(28);
    m_capturedBlackPanel->setMaximumHeight(32);
    ...
    // 中間間距
    rightTimePanelLayout->addSpacing(20);

    // 白方時間標籤 - 初始隱藏（時間在上，進度條在下，一致性排列）
...
void Qt_Chess::updateCapturedPiecesDisplay() {
    ...
    const int sameTypeOffset = pieceSize / 4;  // 相同類型棋子的重疊偏移量（左右覆蓋）
    const int diffTypeOffset = pieceSize;  // 不同類型棋子之間的間距（不重疊）

    // 按棋子類型分組並水平顯示的輔助函數（左右覆蓋）
    auto displayCapturedPiecesHorizontal = [pieceSize, sameTypeOffset, diffTypeOffset](
        ...
        int xPos = 5;  // 起始 x 位置（水平佈局）
        int panelHeight = panel->height();
        // 如果面板高度尚未計算，使用最小高度
        if (panelHeight <= 0) {
            panelHeight = panel->minimumHeight();
            if (panelHeight <= 0) panelHeight = 30;  // 後備最小高度
        }
        int yPos = (panelHeight - pieceSize) / 2;  // 垂直居中
        if (yPos < 0) yPos = 0;  // 確保不為負
        int panelWidth = panel->width();
        // 如果面板寬度尚未計算，使用最小寬度
        if (panelWidth <= 0) {
            panelWidth = panel->minimumWidth();
            if (panelWidth <= 0) panelWidth = 100;  // 後備最小寬度
        }
        ...
        for (size_t i = 0; i < sortedPieces.size(); ++i) {
            ...
            // 檢查是否超出面板寬度範圍
            if (xPos + pieceSize > panelWidth) {
                // 如果超出範圍，仍然放置但會被裁切
                // 這是一個合理的行為，因為在極端情況下（吃了太多棋子）無法完全顯示
            }

            // 放置棋子標籤（水平佈局，左右覆蓋，垂直居中）
