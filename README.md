...
- **地雷模式** 💣🆕：
  - **增加遊戲刺激性**：在棋盤上隨機放置地雷，為傳統西洋棋增添新挑戰
  - **可自訂地雷數量**：使用滑桿調整地雷數量（1-20 個），預設 8 個
  - **智能放置**：地雷只會出現在棋盤中間 4 行的空格，不會影響起始棋子位置
  - **踩雷懲罰**：棋子移動到有地雷的格子會被炸毀，並計入被吃棋子
  - **數字提示**：已揭開的空格會顯示周圍地雷數量（1-8），使用不同顏色區分
  - **視覺標示**：已觸發的地雷會以 💣 符號顯示，並有紅色背景高亮
  - **隨時啟用**：可在遊戲設定中隨時開啟或關閉地雷模式
  - **設定保存**：地雷模式開關和地雷數量設定會在程式關閉後持續保存
- **棋譜記錄與回放**：
...
16. **棋盤反轉**：點擊「設定」選單 >「反轉棋盤」以進行：
    ...
    - 棋盤方向會在程式關閉後儲存並恢復
17. **地雷模式**：在右側遊戲設定面板中啟用：
    - **啟用地雷模式**：勾選「💣 地雷模式」旁的「啟用」複選框
    - **調整地雷數量**：使用滑桿設定地雷數量（1-20 個，預設 8 個）
    - **開始遊戲**：點擊「開始對弈」按鈕後，地雷會隨機放置在棋盤中間區域
    - **遊戲提示**：
      - 棋子移動到地雷格會被炸毀並顯示警告訊息
      - 空格會顯示周圍地雷數量（1-8），數字顏色依數量而異
      - 已觸發的地雷會顯示 💣 符號和紅色背景
      - 被炸毀的棋子會加入被吃棋子列表
    - **策略建議**：避開可疑區域，利用數字提示推測地雷位置
    - 設定會自動儲存，下次開啟程式時保持
18. **棋譜與回放**：
