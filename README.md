...
- **棋譜記錄與回放**：
  ...
  - **遊戲回放功能**：
    ...
    - **計時器在退出回放後自動恢復**
- **吃子分數追蹤**：
  - **即時分數顯示**：右側面板顯示雙方的吃子累計分數
  - **標準棋子價值**：兵=1、騎士=3、主教=3、城堡=5、皇后=9
  - **自動計分**：每次吃子後分數自動更新
  - **支援特殊吃法**：正確處理吃過路兵等特殊情況的計分
  - **新遊戲重置**：開始新遊戲時分數自動歸零
  - 參閱 [SCORING_FEATURE.md](SCORING_FEATURE.md) 以獲得詳細說明
...
14. **吃子分數追蹤**：
    - **查看分數**：右側面板的「吃子分數」區域會即時顯示雙方累計的吃子分數
    - **分數計算**：
      - 兵 = 1 分
      - 騎士 = 3 分
      - 主教 = 3 分
