...
- **線上對戰模式** 🆕：
  - **支援本地網絡和跨網域連線**：同一WiFi或不同網路都可對弈
  - **域名支援**：支援使用域名連線，不限於IP地址
  - **簡易連線碼系統**：一鍵複製貼上的連線碼（格式：IP:房號）
  - **超簡單操作**：創建房間後複製連線碼，朋友貼上即可加入
  - **自動填入功能**：支援剪貼簿貼上，自動解析IP和房號
  - **友善介面**：適合不懂電腦的使用者
  ...
  - 📖 [跨網域連線指南](docs/CROSS_DOMAIN_CONNECTION_GUIDE.md) - 跨網域連線設置教學
