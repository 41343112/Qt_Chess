...
- **吃子記錄顯示**：
  ...
  - 支持 Unicode 符號和自定義圖標
  - 實時更新，直觀呈現對局進展
  - 參閱 [吃子紀錄預覽](docs/CAPTURED_PIECES_PREVIEW.md) 以查看視覺範例
...
13. **棋盤反轉**：點擊「設定」選單 >「反轉棋盤」以進行：
    ...
    - 棋盤方向會在程式關閉後儲存並恢復
14. **吃子記錄查看**：
    - **自動顯示**：被吃掉的棋子自動顯示在棋盤兩側
    - **左側顯示**：白方吃掉的黑子（在黑方時間下方）
    - **右側顯示**：黑方吃掉的白子（在白方時間上方）
    - **智能排列**：相同類型的棋子重疊顯示，不同類型的棋子分開顯示
    - **查看預覽**：參閱 [吃子紀錄預覽](docs/CAPTURED_PIECES_PREVIEW.md) 以了解顯示效果
15. **棋譜與回放**：
